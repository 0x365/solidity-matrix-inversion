...
async function main() {  
    ...
    //tx = await dapp.connect(await key_to_signer(wallets["a"]["private"]))._compute_beta(s_matrix, 0, 1, tx_params);
    // for (let i = 0; i < tx.length; i++) {
    //     console.log(parseInt(tx[i]));
    // }

    //tx = await dapp.connect(await key_to_signer(wallets["a"]["private"]))._build_i_j_list(5);
    tx0 = await dapp.connect(await key_to_signer(wallets["a"]["private"]))._compute_beta(s_matrix, tx_params);

    console.log(tx0);
    console.log("Beta Computed Fine")
    console.log("----------------------------------------");

    tx = await dapp.connect(await key_to_signer(wallets["a"]["private"]))._compute_alpha(s_matrix, b, tx_params);
    console.log(tx);
    // out = []
